...
void cargarDesdeArchivo(std::vector<Contacto>& base_de_datos) {
    base_de_datos.clear(); // Clear existing data

    std::ifstream archivo("base_de_datos.txt");
    if (archivo.is_open()) {
        std::string nombre, apellidos, email;
        int edad;
        while (archivo >> nombre >> apellidos >> edad >> email) {
            Contacto contacto;
            contacto.nombre = nombre;
            contacto.apellidos = apellidos;
            contacto.edad = edad;
            contacto.email = email;
            base_de_datos.push_back(contacto);
        }
        archivo.close();
        std::cout << "Registros cargados desde base_de_datos.txt correctamente." << std::endl;
    } else {
